...
# ------------------------------------------------------------------------------
# Combined Auditd & Rsyslog Setup Script with Enhanced Error Reporting
# This script configures auditd to log to local3 and sets up rsyslog to:
#   - Discard kernel messages.
#   - For auditd execve messages, combine all arguments into a single "command" field.
#   - Forward audit logs (facility local3) to a SIEM server.
#
# Usage: sudo bash setup_logging_combined.sh <SIEM_IP> <SIEM_PORT>
# ------------------------------------------------------------------------------

# Trap unexpected errors and print the line number
...
# Logging function: writes to both STDOUT and LOG_FILE
log() {
    echo -e "$(date '+%Y-%m-%d %H:%M:%S') $1" | tee -a "$LOG_FILE"
}

# Error exit: prints error message (with red color) and exits
error_exit() {
...
# ------------------------------------------------------------------------------
# Determine Distribution and Syslog File
# ------------------------------------------------------------------------------
...
install_packages
log "${GREEN}Packages installed successfully.${NC}"
...
# Backup and update auditd.conf to set log_facility = local3
...
# Backup existing audit rules file if exists
...
log "auditd service restarted and enabled."

# ------------------------------------------------------------------------------
# Configure audisp-syslog plugin to use local3
# ------------------------------------------------------------------------------
...
# 1. Discards kernel messages.
# 2. For messages from facility local3:
#    - If message contains "type=EXECVE", combine execve arguments into a single "command" field.
#    - Forward the message to the SIEM server over TCP.
cat <<EOF > "$SYSLOG_CONF"
...
# Forward all other messages (non-kernel) if needed
if \$syslogfacility-text != "kern" then {
    action(type="omfwd" target="$SIEM_IP" port="$SIEM_PORT" protocol="tcp")
    stop
}
...
log "rsyslog service restarted and enabled."
...
diagnose_rsyslog() {
    ...
    if ! systemctl is-active --quiet rsyslog; then
        ...
        systemctl start rsyslog >> "$LOG_FILE" 2>&1 || { log "Failed to start rsyslog."; return; }
...
diagnose_auditd() {
    ...
    if ! systemctl is-active --quiet auditd; then
        ...
        systemctl start auditd >> "$LOG_FILE" 2>&1 || { log "Failed to start auditd."; return; }
...
if systemctl is-active --quiet auditd && systemctl is-active --quiet rsyslog; then
    ...
    log "2. Running commands that trigger execve and confirming logs show a single command field."
    log "3. Confirming kernel logs are discarded."
else
    error_exit "One or more services are not running. Please review the configuration."
fi

# Send a test message to syslog
log "Sending test message to local syslog..."
...
# Test audit log by modifying /etc/passwd (touch operation)
