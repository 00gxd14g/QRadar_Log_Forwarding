...
execute_cmd() {
    ...
    shift # Remove the description from the arguments
    local cmd=("$@") # Capture all remaining arguments as the command and its arguments
...
retry_operation() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if execute_cmd "$description (attempt $attempt/$max_attempts)" "${cmd[@]}"; then
            return 0
        fi
        attempt=$((attempt + 1))
...
check_prerequisites() {
    ...
    local mutable_audit_rules=false
    local siem_ip=""
    local siem_port=""
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --mutable)
                mutable_audit_rules=true
                shift
                ;;
            -*)
                error_exit "Bilinmeyen seçenek: $1"
                ;;
            *)
                if [ -z "$siem_ip" ]; then
                    siem_ip="$1"
                elif [ -z "$siem_port" ]; then
                    siem_port="$1"
                else
                    warn "Ek argüman göz ardı edildi: $1"
                fi
                shift
                ;;
        esac
    ...
    # Validate SIEM IP and Port
    if [ -z "$siem_ip" ] || [ -z "$siem_port" ]; then
        echo "Kullanım: $0 <SIEM_IP> <SIEM_PORT> [--mutable]" >&2
        error_exit "Gerekli parametreler eksik."
    fi

    # Validate IP address
    if ! echo "$siem_ip" | grep -Eq '^([0-9]{1,3}\.){3}[0-9]{1,3}
...
install_packages() {
    ...
    case "$DISTRO_FAMILY" in
        debian)
            ...
            if ! retry_operation "Package cache update" apt-get update -y; then
                warn "Package cache güncellemesi başarısız, devam ediliyor..."
            fi
            
            # Install base packages
            if ! retry_operation "Base package installation" DEBIAN_FRONTEND=noninteractive apt-get install -y "$base_packages"; then
                error_exit "Temel paketler kurulamadı"
            fi
            
            # Try to install optional packages
            if ! execute_cmd "Extra package installation" DEBIAN_FRONTEND=noninteractive apt-get install -y "$extra_packages"; then
...
load_audit_rules() {
    ...
    if ! execute_cmd "Enable auditd service" systemctl enable auditd; then
...
configure_selinux() {
    ...
    if [ "$selinux_status" = "Enforcing" ] || [ "$selinux_status" = "Permissive" ]; then
        ...
        if command -v setsebool >/dev/null 2>&1; then
            if ! execute_cmd "Enable rsyslog network connections" setsebool -P syslogd_can_network_connect on; then
        ...
        if command -v chcon >/dev/null 2>&1; then
            if ! execute_cmd "Set SELinux context for Python script" chcon -t syslogd_script_exec_t "$PYTHON_SCRIPT_PATH"; then
        ...
        if command -v setsebool >/dev/null 2>&1; then
            if ! execute_cmd "Enable auditd file reading" setsebool -P auditd_can_read_all_files on; then
...
configure_firewall() {
    ...
    if ! execute_cmd "Add firewall rule for SIEM port" firewall-cmd --permanent --add-port="$siem_port/tcp"; then
        warn "Firewall kuralı eklenemedi"
    fi
    
    if ! execute_cmd "Reload firewall rules" firewall-cmd --reload; then
...
configure_debian_specifics() {
    ...
    if command -v ufw >/dev/null 2>&1 && ufw status | grep -q "Status: active"; then
        ...
        if ! execute_cmd "Add UFW rule for SIEM" ufw allow out "$2/tcp"; then
...
test_services() {
    ...
    if [ ${#failed_services[@]} -gt 0 ]; then
        ...
        for service in "${failed_services[@]}"; do
            ...
            execute_cmd "Start $service service" systemctl start "$service" || warn "$service başlatılamadı"
        done
    fi
}
...
main "$@"
; then
        error_exit "Geçersiz SIEM IP adresi: $siem_ip"
    fi

    # Validate port number
    if ! echo "$siem_port" | grep -Eq '^[0-9]+
...
# Execute main function with all arguments
main "$@"
 || [ "$siem_port" -lt 1 ] || [ "$siem_port" -gt 65535 ]; then
        error_exit "Geçersiz SIEM port numarası: $siem_port"
    fi

    # Export for global access
    export SIEM_IP
    export SIEM_PORT
    export MUTABLE_AUDIT_RULES

    SIEM_IP="$siem_ip"
    SIEM_PORT="$siem_port"
    MUTABLE_AUDIT_RULES="$mutable_audit_rules"
